...
def gen_data(train_dist="mesh"):
    ...
    y_test = np.random.uniform(A0-0.7*D, A0+0.7*D, [np.int(M*test_ratio),
                                                    FBGN])

    # broadcast shape: N, M, FBGN
    X_train = np.sum(R(A[None, :, None], y_train[:, None, :], I[None, None, :],
                       dA[None, None, :]), axis=-1)
    X_test = np.sum(R(A[None, :, None], y_test[:, None, :], I[None, None, :],
                      dA[None, None, :]), axis=-1)
...
def plot_datapoint(X_train, y_train, X_test, y_test):
    ...
    N_datapoint = 1
    plt.plot(A/n, R(A[:, None], y_test[N_datapoint, None, 0], I[None, 0],
                    dA[None, 0]), linestyle='dashed', label="FBG1")
    plt.plot(A/n, R(A[:, None], y_test[N_datapoint, None, 1], I[None, 1],
                    dA[None, 1]), linestyle='dashed', label="FBG2")
